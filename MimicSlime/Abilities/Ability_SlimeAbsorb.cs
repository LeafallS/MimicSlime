...
namespace MimicSlime
{
    public class Ability_SlimeAbsorb : Ability
    {
        ...
        public override bool CanApplyOn(LocalTargetInfo target)
        {
            ...
            if (!target.Cell.InHorDistOf(pawn.Position, def.verbProperties.range) || !JumpUtility.ValidJumpTarget(pawn, targetPawn.Map, target.Cell))
            {
                Messages.Message(
            ...
            if (MechanitorUtility.IsMechanitor(targetPawn))
            {
                Messages.Message(
        ...
        public override bool Activate(LocalTargetInfo target, LocalTargetInfo dest)
        {
            ...
            MimicSlimeUtility.AddAbsorbedThought(targetPawn, originalSlime);
            if (ModsConfig.RoyaltyActive && targetPawn.HasPsylink) AbsorbPsy(targetPawn, pawn);
            if (ModsConfig.IdeologyActive)
            {
                targetPawn.ideo.SetIdeo(originalSlime.ideo.Ideo);
            }
