...
namespace MimicSlime
{
    public class MimicSlimeGlobalManager : WorldComponent
    {
        ...
        public void AddPawnToQueueAndRemovePawn(Pawn slime, Pawn pawn, bool strip = true)
        {
            MimicSlimeUtility.RemovePawn(pawn, strip);
        ...
        public override void ExposeData()
        {
            ...
            if (Scribe.mode == LoadSaveMode.Saving)
            {
                ...
                /* ---------- SlimesHair  <Pawn, int>  ---------- */
                slimesHair_keys = SlimesHair.Keys.ToList();
                slimesHair_values = SlimesHair.Values.ToList();

                foreach (var p in originalSlimeQueues_keys.FindAll(p => p != null && p.Dead))
                {
                ...
                foreach (var li in originalSlimeQueues_values)
                {
                    foreach (var p in li.FindAll(p => p != null && p.Dead))
                    {
                        MimicSlimeUtility.ResurrectionThenCureAll(p);
                        if (p.Spawned)
                        {
        ...
        {
            Log.Message("=== MimicSlimeGlobalManager Debug Info ===");
            Log.Message($"Stage: {Scribe.mode}");
            if (originalSlimeQueues_keys == null)
            {
                Log.Message("originalSlimeQueues_keys null");
            }
            else
            {
                foreach (var p in originalSlimeQueues_keys)
                {
                    if (p == null)
                    {
                        Log.Message("null slime");
                    }
                    else
                    {
                        Log.Message($"Slime: {p.NameFullColored}");
                    }
                }
            }

            Log.Message($"OriginalSlimeQueues Count: {originalSlimeQueues.Count}");
            foreach (var kvp in originalSlimeQueues)
            {
                Log.Message($"Slime: {kvp.Key.NameFullColored}, Queue Count: {kvp.Value.Count}");
            }
        }
    }
}