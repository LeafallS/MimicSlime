...
public static class MimicSlimeUtility
{
    ...
    public static void RemovePawn(Pawn pawn, bool strip = true)
    {
        if (pawn == null) return;

        // 卸下所有植入体,剥光装备
        if (pawn.Spawned && strip)
        {
            StripPawn(pawn);
        }
    ...
    {
        foreach (var p in PawnsFinder.All_AliveOrDead)
                yield return p;
    }
    ...
    public static void AddAbsorbedThought(Pawn victim, Pawn billDoer)
    {
        Thought_Memory thought;
        if (ModsConfig.IdeologyActive)
        {
            Find.HistoryEventsManager.RecordEvent(new HistoryEvent(MimicSlimeDefOf.MimicSlimeAbsorbEvent, billDoer.Named(HistoryEventArgsNames.Doer)));
        }
        else
        {
            foreach (Pawn p in PawnsFinder.AllMaps_FreeColonistsSpawned)
            {
                ...
                thought = ThoughtMaker.MakeThought(MimicSlimeDefOf.ColonistAbsorbedByDeliria_Abhorrent, 0);
    ...
    public static void AddPawnToWorld(Pawn pawn)
    {
        ...
        if (pawn.workSettings != null && pawn.workSettings.EverWork)
        {
            ...
            {
                priorities[workTypeDef] = pawn.workSettings.GetPriority(workTypeDef);
            }
        }
        ...
        foreach (KeyValuePair<WorkTypeDef, int> keyValuePair in priorities)
        {
            if (!pawn.WorkTypeIsDisabled(keyValuePair.Key))
            {
                pawn.workSettings.SetPriority(keyValuePair.Key, keyValuePair.Value);
            }
        }
    }

    public static void TerrorSlimeChange(Pawn slime)
    {
        if (slime == null || !IsOriginalSlime(slime)) return;
        if (MimicSlimeModSettings.CognitionFilter)
        {
            if (slime.health.hediffSet.TryGetHediff(MimicSlimeDefOf.TerrorMimicSlime, out Hediff terrorHediff))
            {
                slime.health.RemoveHediff(terrorHediff);
                slime.Drawer.renderer.SetAllGraphicsDirty();
            }
        }
        else
        {
            slime.health.AddHediff(MimicSlimeDefOf.TerrorMimicSlime);
            slime.Drawer.renderer.SetAllGraphicsDirty();
        }
    }
