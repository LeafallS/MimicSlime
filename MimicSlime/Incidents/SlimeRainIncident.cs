...
namespace MimicSlime
{
    ...
    {
        // 事件参数
        public const float BaseWealthThreshold = 10000f;   // 基础财富阈值
        private const int MinDurationTicks = 30000;       // 最小持续时间(0.5天)
        private const int MaxDurationTicks = 90000;      // 最大持续时间(1.5天)
        ...
        protected override bool TryExecuteWorker(IncidentParms parms)
        {
            ...
            condition.map = map;
            Faction faction = FactionUtility.DefaultFactionFrom(MimicSlimeDefOf.HostileWildSlime);
            if (faction != null)
            {
                condition.lord = LordMaker.MakeNewLord(faction, new LordJob_AssaultColony(faction, canTimeoutOrFlee: false), map);
            }
    ...
    public class GameCondition_SlimeRain : GameCondition
    {
        ...
        {
            return DefDatabase<WeatherDef>.GetNamed("Rain");
        }
        ...
        public override void GameConditionTick()
        {
            ...
            // 计算基于财富的生成概率
            float spawnChance = Mathf.Clamp(map.wealthWatcher.WealthTotal / IncidentWorker_SlimeRain.BaseWealthThreshold, 5, 50)/100;

            if (Rand.Chance(spawnChance * MimicSlimeModSettings.SlimeRainChance / 100))
            {
                if (TryFindSpawnPoint(map, out IntVec3 spawnPoint))
                {
                    ...
                    else if (value <= 11 && MimicSlimeModSettings.SpawnGiantSlimeInSlimeRain) // 5% 
                    {
        ...
        private void SpawnSlime(Map map, IntVec3 cell, PawnKindDef slimeKind)
        {
            ...
            if (MimicSlimeModSettings.SlimeRainAssault)
            {
                lord.AddPawn(slime);
                slime.mindState.duty = new PawnDuty(DutyDefOf.AssaultColony);
            }
        }
